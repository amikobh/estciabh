...
coord_furto <- read.csv(file.path(dir_base, "COORDENADAS_FURTO.csv")) %>%
...
pontos_barreiro <- st_intersection(coord_furto, st_buffer(barreiro_shp, 0.01))
...
ggplot() +
  ...
  scale_size_continuous(
    ...
    range = c(3, 15),     # Tamanho mínimo e máximo
  ...
  # Elementos gráficos
  labs(
    title = "Distribuição de furto em Belo Horizonte",
...
output_path <- file.path(dir_base, "atual/mapa_furto.png")
ggsave(
