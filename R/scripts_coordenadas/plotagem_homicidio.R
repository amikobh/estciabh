...
coord_homicidio_sf <- st_as_sf(coord_homicidio, coords = c("longitude", "latitude"), crs = 4326)

# Extrair coordenadas e criar colunas separadas
coord_homicidio_sf <- coord_homicidio_sf %>%
...
mapa <- ggplot() +
  ...
  geom_sf(data = coord_homicidio_sf, color = "red", size = 2.5, alpha = 0.7) +  # Pontos de homicídios
