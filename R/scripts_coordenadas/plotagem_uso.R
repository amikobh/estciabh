...
coord_uso_sf <- st_as_sf(coord_uso, coords = c("longitude", "latitude"), crs = 4326)

# Extrair coordenadas e criar colunas separadas
coord_uso_sf <- coord_uso_sf %>%
...
mapa <- ggplot() +
  ...
  geom_sf(data = coord_uso_sf, color = "red", size = 2.5, alpha = 0.7) +  # Pontos de homicídios
