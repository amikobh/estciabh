...
# --- 3. Carregar e processar dados de homicídio ---
...
print(paste("Pontos de homicídio na região do Barreiro:", nrow(pontos_barreiro)))
...
ggplot() +
  ...
  # Pontos de homicídio com tamanho e cor proporcional ao total
  geom_sf(
    data = coord_homicidio,
    aes(size = total, fill = total),
    shape = 21,           # Losango preenchido
    color = "black",
    stroke = 0.5,
    alpha = 0.9
  ) +
  ...
  # Elementos gráficos
  labs(
    title = "Distribuição espacial: Homicídios em Belo Horizonte",
    subtitle = "Tamanho e cor dos símbolos representam número de casos no local",
