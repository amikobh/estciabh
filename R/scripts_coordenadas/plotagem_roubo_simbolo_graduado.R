...
coord_roubo <- read.csv(file.path(dir_base, "COORDENADAS_ROUBO.csv")) %>%
...
pontos_barreiro <- st_intersection(coord_roubo, st_buffer(barreiro_shp, 0.01))
...
ggplot() +
  ...
  scale_size_continuous(
    ...
    range = c(3, 12),     # Tamanho mínimo e máximo
    breaks = c(1, 10, 30, 60, 90)
  ) +
  ...
  # Elementos gráficos
  labs(
    title = "Distribuição de roubo em Belo Horizonte",
...
output_path <- file.path(dir_base, "atual/mapa_roubo.png")
ggsave(
