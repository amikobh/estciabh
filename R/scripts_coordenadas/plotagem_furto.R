...
coord_furto_sf <- st_as_sf(coord_furto, coords = c("longitude", "latitude"), crs = 4326)

# Extrair coordenadas e criar colunas separadas
coord_furto_sf <- coord_furto_sf %>%
...
mapa <- ggplot() +
  ...
  geom_sf(data = coord_furto_sf, color = "red", size = 2.5, alpha = 0.7) +  # Pontos de homicídios
