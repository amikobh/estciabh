...
coordenadas <- read.csv("COORDENADAS_TRAFICO_DE_DROGAS.csv",header=TRUE, sep=",", dec=".", encoding = "UTF-8", skip = 0) ##Lendo arquivo de texto separado por vírgulas (CSV) e que usa o ponto.
...
bh <- read_municipality(code_muni = 3106200, year = 2023)

#Agregação:
  # Cria uma grade para agrupar os pontos (ajuste o tamanho da célula conforme necessário)
  grid <- st_make_grid(bh, cellsize = 1000)

  dados_sf <- st_as_sf(dados_sf)
  grid <- st_as_sf(grid)

  # Agrupa os pontos na grade
dados_agrupados <- st_join(grid, dados_sf)

#Criação do Mapa de Calor:
  # Calcula a densidade de pontos por célula
  dados_agrupados$densidade <- lengths(dados_agrupados$geometry)
