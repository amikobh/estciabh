...
criar_e_salvar_grafico <- function(tipo, df = NULL, titulo, subtitulo = "", caption = "FONTE: VARA INFRACIONAL/SUASE/DOPCAD",
  ...
  if (tipo == "composto") {
    ...
    grafico <- ggdraw(add_sub(grafico, caption, size = 10, vjust = 1))
    grafico <- grafico + theme(plot.margin = margin(8, 10, 8, 10))
    ...
    if (tipo == "linha") {
      ...
      legend_title <- list(...)$legend_title %||% "PRIMÁRIO"

      # Suporte para múltiplas colunas no fill
      if (length(fill_col) > 1) {
        fill_col_name <- paste(fill_col, collapse = "_")
        df[[fill_col_name]] <- apply(df[, fill_col], 1, paste, collapse = " - ")
      } else {
        fill_col_name <- fill_col
      }

      grafico <- ggplot(df, aes_string(fill = fill_col_name, y = y_col, x = x_col)) +
        geom_bar(position = "dodge", stat = "identity") +
        labs(title = titulo_completo, subtitle = subtitulo, caption = caption, x = "", y = NULL, fill = legend_title) +
        theme(
      ...
      lab_size <- list(...)$lab_size %||% 4
      lab_color <- list(...)$lab_color %||% "white"
      lab_face <- list(...)$lab_face %||% "plain"
      lab_pos <- list(...)$lab_pos %||% "in"
      legend_title <- list(...)$legend_title %||% "Categorias"
      ...
      grafico <- ggpie(
        ...
        label = label_col,
        lab.pos = lab_pos,
        lab.font = list(color = lab_color, size = lab_size, face = lab_face),
        lab.adjust = 0,
        fill = fill_col,
        color = "white",
        palette = paleta
...
criar_e_salvar_grafico(
  tipo = "linha",
  df = df_atendimento,
  titulo = "Atendimento de adolescentes, 2015 a 2024",
...
criar_e_salvar_grafico(
  tipo = "barra_vertical",
  df = passagem_cia_GERAL_HOM_LET_TABELA_TOTAL_bkp,
  titulo = "Óbitos por homicídio e passagem pelo CIABH, Belo Horizonte e municípios limítrofes, 2020 a 2023",
  subtitulo = "Letalidade",  # Subtítulo personalizado
  caption = "FONTE: INSTITUTO MÉDICO LEGAL",
  x_col = "ANO",
  y_col = "QUANTIDADE",
...
criar_e_salvar_grafico(
  tipo = "barra_vertical",
  df = passagem_cia_GERAL_IGN_LET_TABELA_TOTAL_bkp,
  titulo = "Óbitos por causa ignorada e passagem pelo CIABH, Belo Horizonte e municípios limítrofes, 2020 a 2023",
  subtitulo = "Letalidade",  # Subtítulo personalizado
  caption = "FONTE: INSTITUTO MÉDICO LEGAL",
  x_col = "ANO",
  y_col = "QUANTIDADE",
