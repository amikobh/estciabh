...
banco_iml_inicial <- read.csv("iml_inicial.csv",header=TRUE, sep="|", dec=".", encoding = "UTF-8") ##Lendo arquivo de texto separado por vírgulas (CSV) e que usa o ponto.
...
colnames(banco_iml_inicial)[4]<-'DATA_SAIDA_IML'
colnames(banco_iml_inicial)[5]<-'CODIGO_PERICIA'

colnames(banco_iml_inicial)[6]<-'NOME'
colnames(banco_iml_inicial)[7]<-'MAE'
colnames(banco_iml_inicial)[8]<-'NASCIMENTO'
colnames(banco_iml_inicial)[9]<-'FOI_DESCONHECIDO'
colnames(banco_iml_inicial)[10]<-'ENDERECO'

colnames(banco_iml_inicial)[11]<-'LATITUDE'
colnames(banco_iml_inicial)[12]<-'LONGITUDE'
colnames(banco_iml_inicial)[13]<-'MUNICIPIO'
colnames(banco_iml_inicial)[14]<-'CAUSA_MORTE'
colnames(banco_iml_inicial)[15]<-'SEXO'

colnames(banco_iml_inicial)[16]<-'COR'
colnames(banco_iml_inicial)[17]<-'CAUSA_JURIDICA'
colnames(banco_iml_inicial)[18]<-'SUB_CAUSA_JURIDICA'
colnames(banco_iml_inicial)[19]<-'PROCEDENCIA'
colnames(banco_iml_inicial)[20]<-'ESPECIFICACAO_PROCEDENCIA'
colnames(banco_iml_inicial)[21]<-'DESCONHECIDO'
#########################################################################################################

library(dplyr)

# Criar a nova coluna ID_1 e fazê-la a primeira coluna
banco_iml_inicial <- banco_iml_inicial %>%
  mutate(ID_1 = seq_along(.[, 1])) %>%
  relocate(ID_1, .before = 1)
...
banco_iml_inicial = banco_iml_inicial %>%
  mutate(NASCIMENTO2 = substr(banco_iml_inicial$NASCIMENTO, start = 1, stop = 10))

banco_iml_inicial$NASCIMENTO2 = dmy(banco_iml_inicial$NASCIMENTO2)
...
#passado por mensagem zap.

library(abjutils)
banco_iml_inicial$DESCONHECIDO2 = ajustar_nomes(banco_iml_inicial$DESCONHECIDO)


banco_iml_inicial_permanece_desconhecido =
  banco_iml_inicial %>%
  filter(DESCONHECIDO2 %in% "1")

banco_iml_inicial =
  banco_iml_inicial %>%
  filter(DESCONHECIDO2 %in% "0")
#########################################################################################################
...
banco_iml_inicial$MUNICIPIO_OCORRENCIA2 = ajustar_nomes(banco_iml_inicial$MUNICIPIO)


banco_iml_inicial$CAUSA_JURIDICA[banco_iml_inicial$CAUSA_JURIDICA == "ACIDENTE-NAOESPECIFICADO"]<- "ACIDENTE-NÃO ESPECIFICADO"
banco_iml_inicial$CAUSA_JURIDICA[banco_iml_inicial$CAUSA_JURIDICA == "HOMICIDIO-"]<- "HOMICÍDIO"
banco_iml_inicial$CAUSA_JURIDICA[banco_iml_inicial$CAUSA_JURIDICA == "HOMICIDIO"]<- "HOMICÍDIO"
banco_iml_inicial$CAUSA_JURIDICA[banco_iml_inicial$CAUSA_JURIDICA == "IGNORADA-"]<- "IGNORADA"
banco_iml_inicial$CAUSA_JURIDICA[banco_iml_inicial$CAUSA_JURIDICA == "NATURAL-"]<- "NATURAL"
banco_iml_inicial$CAUSA_JURIDICA[banco_iml_inicial$CAUSA_JURIDICA == "SUICIDIO-"]<- "SUICÍDIO"
banco_iml_inicial$CAUSA_JURIDICA[banco_iml_inicial$CAUSA_JURIDICA == "SUICIDIO"]<- "SUICÍDIO"

#AGREGAR ACIDENTE
banco_iml_inicial$CAUSA_JURIDICA <- str_replace(banco_iml_inicial$CAUSA_JURIDICA, "ACIDENT.*", "ACIDENTE")
banco_iml_inicial$CAUSA_JURIDICA <- str_replace(banco_iml_inicial$CAUSA_JURIDICA, "HOMIC.*", "HOMICÍDIO")
banco_iml_inicial$CAUSA_JURIDICA <- str_replace(banco_iml_inicial$CAUSA_JURIDICA, "SUIC.*", "SUICÍDIO")
banco_iml_inicial$CAUSA_JURIDICA <- str_replace(banco_iml_inicial$CAUSA_JURIDICA, "NATU.*", "NATURAL")
...
banco_PARA_CAI = banco_iml_inicial
...
summary(banco_PARA_CAI$ENTRADA2)
#########################################################################################################
#########################################################################################################
...
banco_COM_CAI_inicial =
  banco_COM_CAI_inicial |>
  relocate(ID_1, ID, .after = last_col())


#fazendo causa_juridica ultima coluna
library(dplyr)

banco_COM_CAI_inicial <- banco_COM_CAI_inicial %>%
  select(-CAUSA_JURIDICA, everything())


#selecionando somentes as colunas do banco que retornou com a CAI incluindo causa_juridica (2025)
banco_COM_CAI_inicial =
  banco_COM_CAI_inicial |>
...
#considerando que houve edições no banco_COM_CAI_inicial (CAUSA_JURÍDICA) manter os dados nesta (2025)

library(dplyr)

banco_iml_GERAL <- banco_iml_GERAL %>%
  mutate(CAUSA_JURIDICA.y = coalesce(CAUSA_JURIDICA.y, CAUSA_JURIDICA.x)) %>%
  select(-CAUSA_JURIDICA.x) %>%
  rename(CAUSA_JURIDICA = CAUSA_JURIDICA.y)
...
#########################################################################################################
#colnames(banco_iml_GERAL)[31]<-'PASSAGEM_CIABH'
banco_iml_GERAL <- banco_iml_GERAL %>%
  rename(PASSAGEM_CIABH = PASSAGEM_CIA)
...
banco_letalidade$PERMANECE_DESCONHECIDO = banco_letalidade$DESCONHECIDO2 # 2025
banco_letalidade$PERMANECE_DESCONHECIDO[banco_letalidade$PERMANECE_DESCONHECIDO == 0] <- "NAO"
...
summary(banco_letalidade$ENTRADA2)
#########################################################################################################
library(abjutils)
...
colnames(CAUSA_JURIDICA_let)[1]<-'CAUSA_JURÍDICA'
...
CAUSA_JURIDICA_let_ANTERIOR20 <- read.csv("CAUSA_JURIDICA_let_ANTERIOR20.csv", header=TRUE, sep=",", dec=".", encoding = "UTF-8", skip = 0) ##Lendo arquivo de texto separado por vírgulas (CSV) e que usa o ponto.
CAUSA_JURIDICA_let_ANTERIOR21 <- read.csv("CAUSA_JURIDICA_let_ANTERIOR21.csv", header=TRUE, sep=",", dec=".", encoding = "UTF-8", skip = 0) ##Lendo arquivo de texto separado por vírgulas (CSV) e que usa o ponto.
CAUSA_JURIDICA_let_ANTERIOR22 <- read.csv("CAUSA_JURIDICA_let_ANTERIOR22.csv", header=TRUE, sep=",", dec=".", encoding = "UTF-8", skip = 0) ##Lendo arquivo de texto separado por vírgulas (CSV) e que usa o ponto.

#acertando os nomes das colunas de todos os dataframes:
# Lista todos os dataframes cujo nome começa com "CAUSA_JURIDICA_let_ANTERIOR"
df_list <- ls(pattern = "^CAUSA_JURIDICA_let_ANTERIOR")
...
for (df_name in df_list) {
  ...
  if (ncol(df) == 3) {
    colnames(df) <- c("CAUSA_JURÍDICA", "QUANTIDADE", "PERCENTUAL")

    # Substitui todos os NA do dataframe por 0
    df[is.na(df)] <- 0

    # Adiciona o símbolo de % ao final da coluna PERCENTUAL
    df$PERCENTUAL <- paste0(df$PERCENTUAL, "%")

    assign(df_name, df)
  } else {
    message(paste("O dataframe", df_name, "não possui exatamente 3 colunas."))
  }
}
...
# Passo 1: Pegar todos os dataframes que começam com "CAUSA_JURIDICA_let_ANTERIOR"
df_names <- ls(pattern = "^CAUSA_JURIDICA_let_ANTERIOR")

# Passo 2: Ordenar os nomes em ordem alfabética
df_names <- sort(df_names)

# Passo 3: Juntar todos com full_join de forma acumulativa
CAUSA_JURIDICA_let_TOTAL <- get(df_names[1])

for (name in df_names[-1]) {
  CAUSA_JURIDICA_let_TOTAL <- full_join(CAUSA_JURIDICA_let_TOTAL, get(name), by = "CAUSA_JURÍDICA")
}

# Passo 4: Juntar com o dataframe final "CAUSA_JURIDICA_let"
CAUSA_JURIDICA_let_TOTAL <- full_join(CAUSA_JURIDICA_let_TOTAL, CAUSA_JURIDICA_let, by = "CAUSA_JURÍDICA")


colnames(CAUSA_JURIDICA_let_TOTAL) <- c("CAUSA JURÍDICA", "2020", "%20", "2021", "%21", "2022", "%22", "2023", "%23")
#########################################################################################################
#########################################################################################################
...
colnames(soma_intervalo_idade_HOMICIDIO_let)[1]<-'FAIXA_ETÁRIA'
...
soma_intervalo_idade_HOMICIDIO_let_ANTERIOR20 <- read.csv("soma_intervalo_idade_HOMICIDIO_let_ANTERIOR20.csv", header=TRUE, sep=",", dec=".", encoding = "UTF-8", skip = 0) ##Lendo arquivo de texto separado por vírgulas (CSV) e que usa o ponto.
soma_intervalo_idade_HOMICIDIO_let_ANTERIOR21 <- read.csv("soma_intervalo_idade_HOMICIDIO_let_ANTERIOR21.csv", header=TRUE, sep=",", dec=".", encoding = "UTF-8", skip = 0) ##Lendo arquivo de texto separado por vírgulas (CSV) e que usa o ponto.
soma_intervalo_idade_HOMICIDIO_let_ANTERIOR22 <- read.csv("soma_intervalo_idade_HOMICIDIO_let_ANTERIOR22.csv", header=TRUE, sep=",", dec=".", encoding = "UTF-8", skip = 0) ##Lendo arquivo de texto separado por vírgulas (CSV) e que usa o ponto.

#acertando os nomes das colunas de todos os dataframes:
# Lista todos os dataframes cujo nome começa com "soma_intervalo_idade_HOMICIDIO_let_ANTERIOR"
df_list <- ls(pattern = "^soma_intervalo_idade_HOMICIDIO_let_ANTERIOR")
...
for (df_name in df_list) {
  ...
  if (ncol(df) == 3) {
    colnames(df) <- c("FAIXA_ETÁRIA", "QUANTIDADE", "PERCENTUAL")
...
# Passo 1: Pegar todos os dataframes que começam com "soma_intervalo_idade_HOMICIDIO_let_ANTERIOR"
df_names_intervalo_idade_HOMICIDIO_let <- ls(pattern = "^soma_intervalo_idade_HOMICIDIO_let_ANTERIOR")

# Passo 2: Ordenar os nomes em ordem alfabética
df_names_intervalo_idade_HOMICIDIO_let <- sort(df_names_intervalo_idade_HOMICIDIO_let)

# Passo 3: Juntar todos com full_join de forma acumulativa
soma_intervalo_idade_HOMICIDIO_let_TOTAL <- get(df_names_intervalo_idade_HOMICIDIO_let[1])

for (name in df_names_intervalo_idade_HOMICIDIO_let[-1]) {
  soma_intervalo_idade_HOMICIDIO_let_TOTAL <- full_join(soma_intervalo_idade_HOMICIDIO_let_TOTAL, get(name), by = "FAIXA_ETÁRIA")
}

# Passo 4: Juntar com o dataframe final "soma_intervalo_idade_HOMICIDIO_let"
soma_intervalo_idade_HOMICIDIO_let_TOTAL <- full_join(soma_intervalo_idade_HOMICIDIO_let_TOTAL, soma_intervalo_idade_HOMICIDIO_let, by = "FAIXA_ETÁRIA")


colnames(soma_intervalo_idade_HOMICIDIO_let_TOTAL) <- c("FAIXA ETÁRIA", "2020", "%20", "2021", "%21", "2022", "%22", "2023", "%23")
#########################################################################################################
#########################################################################################################
setwd(file.path("~/diretorio_r/estciabh/R/"))#configurar diretorio
#########################################################################################################
...
colnames(soma_intervalo_idade_IGNORADA_let)[1]<-'FAIXA_ETÁRIA'
...
soma_intervalo_idade_IGNORADA_let_ANTERIOR20 <- read.csv("soma_intervalo_idade_IGNORADA_let_ANTERIOR20.csv", header=TRUE, sep=",", dec=".", encoding = "UTF-8", skip = 0) ##Lendo arquivo de texto separado por vírgulas (CSV) e que usa o ponto.
soma_intervalo_idade_IGNORADA_let_ANTERIOR21 <- read.csv("soma_intervalo_idade_IGNORADA_let_ANTERIOR21.csv", header=TRUE, sep=",", dec=".", encoding = "UTF-8", skip = 0) ##Lendo arquivo de texto separado por vírgulas (CSV) e que usa o ponto.
soma_intervalo_idade_IGNORADA_let_ANTERIOR22 <- read.csv("soma_intervalo_idade_IGNORADA_let_ANTERIOR22.csv", header=TRUE, sep=",", dec=".", encoding = "UTF-8", skip = 0) ##Lendo arquivo de texto separado por vírgulas (CSV) e que usa o ponto.

#acertando os nomes das colunas de todos os dataframes:
# Lista todos os dataframes cujo nome começa com "soma_intervalo_idade_IGNORADA_let_ANTERIOR"
df_list <- ls(pattern = "^soma_intervalo_idade_IGNORADA_let_ANTERIOR")
...
# Passo 1: Pegar todos os dataframes que começam com "soma_intervalo_idade_IGNORADA_let_ANTERIOR"
df_names_intervalo_idade_IGNORADA_let <- ls(pattern = "^soma_intervalo_idade_IGNORADA_let_ANTERIOR")

# Passo 2: Ordenar os nomes em ordem alfabética
df_names_intervalo_idade_IGNORADA_let <- sort(df_names_intervalo_idade_IGNORADA_let)

# Passo 3: Juntar todos com full_join de forma acumulativa
soma_intervalo_idade_IGNORADA_let_TOTAL <- get(df_names_intervalo_idade_IGNORADA_let[1])

for (name in df_names_intervalo_idade_IGNORADA_let[-1]) {
  soma_intervalo_idade_IGNORADA_let_TOTAL <- full_join(soma_intervalo_idade_IGNORADA_let_TOTAL, get(name), by = "FAIXA_ETÁRIA")
}

# Passo 4: Juntar com o dataframe final "soma_intervalo_idade_IGNORADA_let"
soma_intervalo_idade_IGNORADA_let_TOTAL <- full_join(soma_intervalo_idade_IGNORADA_let_TOTAL, soma_intervalo_idade_IGNORADA_let, by = "FAIXA_ETÁRIA")


colnames(soma_intervalo_idade_IGNORADA_let_TOTAL) <- c("FAIXA ETÁRIA", "2020", "%20", "2021", "%21", "2022", "%22", "2023", "%23")
#########################################################################################################
#########################################################################################################
setwd(file.path("~/diretorio_r/estciabh/R/"))#configurar diretorio
#########################################################################################################
...
#########################################################################################################
# banco_HOMICIDIO_geral_faixa_etaria
#########################################################################################################
...
banco_HOMICIDIO_geral_faixa_etaria_2022 <- data.table(IDADE = c(12, 15,16, 17, 18, 19, 20),
                                                      QUANTIDADE = c(1, 4, 8, 9, 13, 26,18))
...
# Se desejar, desanexe o pacote plyr para evitar conflitos:
# detach("package:plyr", unload = TRUE)

# 1. Lista os nomes dos data frames que iniciam com o padrão desejado
lista_nomes <- ls(pattern = "^banco_HOMICIDIO_geral_faixa_etaria")

# 2. Separa o data frame ATUAL dos demais
nome_atual  <- grep("ATUAL$", lista_nomes, value = TRUE)
outros_nomes <- setdiff(lista_nomes, nome_atual)

# 3. Ordena os nomes (alfabeticamente) e coloca o ATUAL por último
nomes_ordenados <- c(sort(outros_nomes), nome_atual)

# 4. Processa cada data frame:
#    - Converte a coluna IDADE para numérico (extraindo somente os dígitos);
#    - Agrupa por IDADE e soma a coluna QUANTIDADE (para evitar duplicatas);
#    - Renomeia a coluna resultante conforme os 4 últimos dígitos extraídos do nome original,
#      ou, se for o data frame ATUAL, usa (ano corrente - 2).
lista_dfs <- mapply(function(nome) {
  col_new <- if (grepl("ATUAL$", nome)) {
    # Se o nome termina com ATUAL, define a nova coluna como ano corrente - 2
    as.character(as.numeric(format(Sys.Date(), "%Y")) - 2)
  } else {
    # Caso contrário, extrai os 4 últimos caracteres do nome original
    substr(nome, nchar(nome) - 3, nchar(nome))
  }

  df <- get(nome) %>%
    mutate(IDADE = as.numeric(gsub("[^0-9]", "", as.character(IDADE)))) %>%
    group_by(IDADE) %>%
    dplyr::summarize(QUANTIDADE = sum(QUANTIDADE), .groups = "drop") %>%
    rename(!!rlang::sym(col_new) := QUANTIDADE)

  return(df)
}, nome = nomes_ordenados, SIMPLIFY = FALSE)

# 5. Realiza o full_join sucessivo pela coluna "IDADE"
df_join <- reduce(lista_dfs, full_join, by = "IDADE")

# 6. Substitui os NA's por 0 em todas as colunas (exceto a coluna de idade)
df_join <- df_join %>%
  mutate(across(-1, ~ replace_na(.x, 0)))

# 7. Ordena pela coluna IDADE e formata os valores para incluir " anos"
df_join <- df_join %>%
  arrange(IDADE) %>%
  mutate(IDADE = paste0(IDADE, " anos"))

# 8. Remove a penúltima coluna (assumindo que as duas últimas colunas são idênticas)
if(ncol(df_join) > 1) {
  df_join <- df_join[, -(ncol(df_join) - 1)]
}

# 9. Adiciona uma linha TOTAL, somando os valores de cada coluna (exceto a coluna IDADE)
total_vals <- colSums(df_join[ , -1])
total_row <- tibble(IDADE = "TOTAL", !!!as.list(total_vals))
tab_letalidade_geral_12_20_HOMICIDIO <- bind_rows(df_join, total_row)

library(dplyr)
library(tidyr)

tab_letalidade_geral_12_20_HOMICIDIO_bkp <- tab_letalidade_geral_12_20_HOMICIDIO %>%
  # Exclui a linha TOTAL
  filter(IDADE != "TOTAL") %>%
  # Converte o formato wide para long:
  pivot_longer(
    cols = -IDADE,       # Todas as colunas, exceto a de IDADE
    names_to = "ANO",    # O nome das colunas vai para a nova coluna ANO
    values_to = "QUANTIDADE" # Os valores vão para a coluna QUANTIDADE
  ) %>%
  # Organiza as linhas se necessário
  arrange(ANO, IDADE)

# Exibe o resultado final
#print(tab_letalidade_geral_12_20_HOMICIDIO)
...
#########################################################################################################
# banco_HOMICIDIO_geral_faixa_etaria FIM
#########################################################################################################
setwd(file.path("~/diretorio_r/estciabh/letalidade"))
...
#########################################################################################################
#########################################################################################################
# banco_IGNORADA_geral_faixa_etaria
#########################################################################################################
...
banco_IGNORADA_geral_faixa_etaria_ATUAL = banco_letalidade_12_20 %>%
...
banco_IGNORADA_geral_faixa_etaria_2018 <- data.table(IDADE = c(12,13, 15,16, 17, 18, 19, 20),
                                                     QUANTIDADE = c(4,2,8,12, 16, 27, 15, 24))
...
banco_IGNORADA_geral_faixa_etaria_2021 <- data.table(IDADE = c(13,14,15,16, 17, 18, 19, 20),
                                                     QUANTIDADE = c(1, 3, 1, 5, 11, 10,17,21))

banco_IGNORADA_geral_faixa_etaria_2022 <- data.table(IDADE = c(12, 13, 14, 16, 17, 18, 19, 20),
                                                     QUANTIDADE = c(3,1,1,5,7,9,3,9))
...
# 1. Lista os nomes dos data frames que iniciam com o padrão desejado
lista_nomes <- ls(pattern = "^banco_IGNORADA_geral_faixa_etaria")
...
total_vals <- colSums(df_join[ , -1])
total_row <- tibble(IDADE = "TOTAL", !!!as.list(total_vals))
tab_letalidade_geral_12_20_IGNORADA <- bind_rows(df_join, total_row)

library(dplyr)
library(tidyr)

tab_letalidade_geral_12_20_IGNORADA_bkp <- tab_letalidade_geral_12_20_IGNORADA %>%
  # Exclui a linha TOTAL
  filter(IDADE != "TOTAL") %>%
  # Converte o formato wide para long:
  pivot_longer(
...
# Exibe o resultado final
#print(tab_letalidade_geral_12_20_IGNORADA)
...
#########################################################################################################
# banco_IGNORADA_geral_faixa_etaria FIM
#########################################################################################################
setwd(file.path("~/diretorio_r/estciabh/letalidade"))
...
banco_SEXO_hom_ign_LETALIDADE$SEXO[banco_SEXO_hom_ign_LETALIDADE$SEXO == "ESTIMADOMASCULINO"]<- "ESTIMADO MASCULINO"
#########################################################################################################
#############################################################################################################
#########################################################################################################
#########################################################################################################
...
########################################################################################################
#########################################################################################################
#passagem_cia_GERAL_HOM_IGN_LET_TABELA_TOTAL
#########################################################################################################
#SEPARANDO SOMENTE VARIAVEIS NECESSARIAS PARA AGILIZAR TRATAMENTO:
library(dplyr)

passagem_cia_GERAL_HOM_IGN_LET_TABELA_2023 = banco_letalidade %>%
  filter(IDADE2 >= 12 & IDADE2<= 20) %>%
  #filter(PASSAGEM_CIABH %in% "SIM") |>
  filter (CAUSA_JURIDICA == "HOMICÍDIO" | CAUSA_JURIDICA == "IGNORADA")  %>%
  select(CAUSA_JURIDICA, PASSAGEM_CIABH)
...
#Contagem considerando as duas colunas
passagem_cia_GERAL_HOM_IGN_LET_TABELA_2023 <- passagem_cia_GERAL_HOM_IGN_LET_TABELA_2023 %>%
  count(CAUSA_JURIDICA, PASSAGEM_CIABH, name = "QUANTIDADE") %>%
  mutate(ANO = 2023)  # Adicionando a coluna ANO preenchida com 2023
...
# Criar o dataframe
passagem_cia_GERAL_HOM_IGN_LET_TABELA_2020 <- data.frame(
  CAUSA_JURIDICA = c("HOMICÍDIO", "HOMICÍDIO", "IGNORADA", "IGNORADA"),
  PASSAGEM_CIABH = c("NAO", "SIM", "NAO", "SIM"),
  QUANTIDADE = c(41, 38, 81, 54),
...
# Criar o dataframe
passagem_cia_GERAL_HOM_IGN_LET_TABELA_2021 <- data.frame(
  CAUSA_JURIDICA = c("HOMICÍDIO", "HOMICÍDIO", "IGNORADA", "IGNORADA"),
  PASSAGEM_CIABH = c("NAO", "SIM", "NAO", "SIM"),
  QUANTIDADE = c(32, 51, 41, 28),
  ANO = c(2021, 2021, 2021, 2021)
)
#########################################################################################################
#########################################################################################################

# Criar o dataframe
passagem_cia_GERAL_HOM_IGN_LET_TABELA_2022 <- data.frame(
  CAUSA_JURIDICA = c("HOMICÍDIO", "HOMICÍDIO", "IGNORADA", "IGNORADA"),
  PASSAGEM_CIABH = c("NAO", "SIM", "NAO", "SIM"),
  QUANTIDADE = c(42, 37, 30, 8),
  ANO = c(2022, 2022, 2022, 2022)
)
#########################################################################################################
# Carregar pacotes
library(dplyr)
...
# Anos disponíveis
anos <- 2020:2023
nomes_dfs <- paste0("passagem_cia_GERAL_HOM_IGN_LET_TABELA_", anos)

# Função para padronizar colunas (sem renomear ainda)
padronizar_df <- function(nome_df) {
  df <- get(nome_df)
  colnames(df) <- toupper(trimws(colnames(df)))  # padronizar nomes (sem renomear ainda)
  return(df)
}

# Aplicar a função e juntar os dataframes
lista_dfs <- lapply(nomes_dfs, padronizar_df)
passagem_total <- do.call(rbind, lista_dfs)

# Salvar backup antes de renomear as colunas
passagem_cia_GERAL_HOM_IGN_LET_TABELA_TOTAL_bkp <- passagem_total

# Agora renomear as duas primeiras colunas
colnames(passagem_total)[1:2] <- c("CAUSA JURÍDICA", "PASSAGEM CIABH")

# Criar o dataframe final com anos como colunas
passagem_cia_GERAL_HOM_IGN_LET_TABELA_TOTAL <- passagem_total %>%
  pivot_wider(
    id_cols = c(`CAUSA JURÍDICA`, `PASSAGEM CIABH`),
    names_from = ANO,
    values_from = QUANTIDADE,
    values_fill = 0
  )

# Criar linha TOTAL com CAUSA JURÍDICA em branco
linha_total <- passagem_cia_GERAL_HOM_IGN_LET_TABELA_TOTAL %>%
  summarise(across(where(is.numeric), sum)) %>%
  mutate(`CAUSA JURÍDICA` = "", `PASSAGEM CIABH` = "TOTAL") %>%
  select(colnames(passagem_cia_GERAL_HOM_IGN_LET_TABELA_TOTAL))

# Adicionar a linha TOTAL
passagem_cia_GERAL_HOM_IGN_LET_TABELA_TOTAL <- bind_rows(
  passagem_cia_GERAL_HOM_IGN_LET_TABELA_TOTAL,
  linha_total
)

# Visualizar resultado
#print(passagem_cia_GERAL_HOM_IGN_LET_TABELA_TOTAL)
#########################################################################################################
passagem_cia_GERAL_HOM_LET_TABELA_TOTAL = passagem_cia_GERAL_HOM_IGN_LET_TABELA_TOTAL |>
  filter(`CAUSA JURÍDICA` %in% "HOMICÍDIO")



library(dplyr)

# Criar a linha TOTAL sem a palavra "HOMICÍDIO"
total_linha <- passagem_cia_GERAL_HOM_LET_TABELA_TOTAL %>%
  summarise(
    `CAUSA JURÍDICA` = "",  # Deixa a célula vazia
    `PASSAGEM CIABH` = "TOTAL",
    across(`2020`:`2023`, sum) # Calcula soma das colunas
  )

# Adicionar a linha TOTAL à tabela original
passagem_cia_GERAL_HOM_LET_TABELA_TOTAL <- bind_rows(passagem_cia_GERAL_HOM_LET_TABELA_TOTAL, total_linha)

# Exibir a tabela resultante
#print(passagem_cia_GERAL_HOM_LET_TABELA_TOTAL)

#########################################################################################################
#########################################################################################################
passagem_cia_GERAL_IGN_LET_TABELA_TOTAL = passagem_cia_GERAL_HOM_IGN_LET_TABELA_TOTAL |>
  filter(`CAUSA JURÍDICA` %in% "IGNORADA")

library(dplyr)

# Criar a linha TOTAL sem a palavra "HOMICÍDIO"
total_linha <- passagem_cia_GERAL_IGN_LET_TABELA_TOTAL %>%
  summarise(
    `CAUSA JURÍDICA` = "",  # Deixa a célula vazia
    `PASSAGEM CIABH` = "TOTAL",
    across(`2020`:`2023`, sum) # Calcula soma das colunas
...
# Adicionar a linha TOTAL à tabela original
passagem_cia_GERAL_IGN_LET_TABELA_TOTAL <- bind_rows(passagem_cia_GERAL_IGN_LET_TABELA_TOTAL, total_linha)

# Exibir a tabela resultante
#print(passagem_cia_GERAL_IGN_LET_TABELA_TOTAL)
#########################################################################################################
#########################################################################################################
passagem_cia_GERAL_HOM_LET_TABELA_TOTAL_bkp = passagem_cia_GERAL_HOM_IGN_LET_TABELA_TOTAL_bkp |>
  filter(CAUSA_JURIDICA %in% "HOMICÍDIO")


#########################################################################################################
passagem_cia_GERAL_IGN_LET_TABELA_TOTAL_bkp = passagem_cia_GERAL_HOM_IGN_LET_TABELA_TOTAL_bkp |>
  filter(CAUSA_JURIDICA %in% "IGNORADA")
#########################################################################################################
#########################################################################################################
#########################################################################################################
#########################################################################################################
#passagem_cia_GERAL_HOM_IGN_LET_TABELA_TOTAL FIM
#########################################################################################################
#########################################################################################################
...
homicidio_sexo_LET = banco_letalidade %>%
  filter(IDADE2 >= 12 & IDADE2 <= 20) %>%
  filter(PASSAGEM_CIABH %in% "SIM") |>
  filter(CAUSA_JURIDICA == "HOMICÍDIO") %>%
  select(SEXO)
...
homicidio_sexo_LET_2023 <- homicidio_sexo_LET %>%
  group_by(SEXO) %>%
  summarise(QUANTIDADE = n()) %>%
  mutate(ANO = 2023)

print(homicidio_sexo_LET_2023)

library(tibble)

homicidio_sexo_LET_2020 <- tibble::tibble(
  SEXO = c("Feminino", "Masculino"),
  QUANTIDADE = c(1, 37),
  ANO = 2020
)

homicidio_sexo_LET_2021 <- tibble::tibble(
  SEXO = c("Feminino", "Masculino"),
  QUANTIDADE = c(0, 51),
  ANO = 2021
)

homicidio_sexo_LET_2022 <- tibble::tibble(
  SEXO = c("Feminino", "Masculino"),
  QUANTIDADE = c(1, 36),
  ANO = 2022
)
...
# Anos disponíveis
anos <- 2020:2023
nomes_dfs <- paste0("homicidio_sexo_LET_", anos)

# Função para padronizar colunas (sem renomear ainda)
padronizar_df <- function(nome_df) {
  df <- get(nome_df)
  colnames(df) <- toupper(trimws(colnames(df)))  # padronizar nomes
  return(df)
}

# Aplicar a função e juntar os dataframes
lista_dfs <- lapply(nomes_dfs, padronizar_df)
homicidio_total <- do.call(rbind, lista_dfs)

# Salvar backup
homicidio_sexo_LET_TOTAL_bkp <- homicidio_total

# Renomear a primeira coluna
colnames(homicidio_total)[1] <- "SEXO"

# Criar o dataframe final com anos como colunas
homicidio_sexo_LET_TOTAL <- homicidio_total %>%
  pivot_wider(
    id_cols = SEXO,
    names_from = ANO,
    values_from = QUANTIDADE,
...
# Criar linha TOTAL com SEXO = "TOTAL"
linha_total <- homicidio_sexo_LET_TOTAL %>%
  summarise(across(where(is.numeric), sum)) %>%
  mutate(SEXO = "TOTAL") %>%
  select(colnames(homicidio_sexo_LET_TOTAL))

# Adicionar a linha TOTAL
homicidio_sexo_LET_TOTAL <- bind_rows(
  homicidio_sexo_LET_TOTAL,
  linha_total
)

# Visualizar resultado
#print(homicidio_sexo_LET_TOTAL)
#########################################################################################################
#homicidio_sexo_LET FIM
#########################################################################################################
#############################################################################################################
