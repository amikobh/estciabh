...
setwd(file.path("~/diretorio_r/estciabh/cedipro/arq_fontes/"))
...
# Identificar as colunas que começam com "data" e são de classe character
colunas_data_chr <- banco_cedipro %>%
  select(starts_with("data")) %>% # Seleciona colunas que começam com "data"
  select(where(is.character)) %>% # Filtra apenas as colunas de classe character
  names() # Obtém os nomes dessas colunas

# Converter as colunas identificadas e a coluna "nascimento" para a classe Date
banco_cedipro <- banco_cedipro %>%
  mutate(
    across(all_of(colunas_data_chr), dmy), # Converte colunas que começam com "data"
    nascimento = dmy(nascimento) # Converte a coluna "nascimento"
  )
...
#############################################################################################################
#banco_adl_encaminhado
#########################################################################################################
#########################################################################################################
# salvando para gráfico
banco_adl_encaminhado_bkp =
  banco_cedipro %>%
  janitor::tabyl(encaminhamento_cedipro) %>%
  arrange(n) %>%
  #arrange(desc(n)) %>%
...
colnames(banco_adl_encaminhado_bkp)[1]<-'banco_adl_encaminhado_bkp'
...
colnames(banco_adl_encaminhado)[1]<-'CURSO'
colnames(banco_adl_encaminhado)[2]<-'QUANTIDADE'
colnames(banco_adl_encaminhado)[3]<-'PERCENTUAL'
...
banco_cedipro <- banco_cedipro %>%
  rename(
    "curso_cedipro_adl_01" = "curso_cedipro_1",
    "curso_cedipro_adl_02" = "curso_cedipro_2",
    "curso_cedipro_adl_03" = "curso_cedipro_3",
    "curso_cedipro_adl_04" = "curso_cedipro_4",
    "curso_cedipro_adl_05" = "curso_cedipro_4_1"
  )
...
REGIONAL_RESIDENCIAL_banco_matriculados_CEDIPRO$regional_residencial <- str_replace(REGIONAL_RESIDENCIAL_banco_matriculados_CEDIPRO$regional_residencial, "CIDADE DE RIO MANSO/MG", "ZREGIÃO METROPOLITANA")
